...
function printInvoice(sale: any, vendor: any, format: 'a4' | 'thermal', settings?: any) {
  ...
  const a4Style = `@page{size:A4;margin:20mm 25mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:14px;color:#000;font-weight:600;width:620px;max-width:100%;margin:0 auto}.header{text-align:center;padding:15px 0;border-bottom:1px dotted #000}.shop-name{font-size:24px;font-weight:900}table{width:100%;border-collapse:collapse;margin:12px 0}th{text-align:left;font-size:12px;font-weight:900;padding:8px 4px;border-bottom:1px solid #000}td{padding:8px 4px;font-size:13px;font-weight:600;border-bottom:1px dotted #999}.text-right{text-align:right}.totals{border-top:1px solid #000;padding-top:8px;margin-top:5px}.total-row{display:flex;justify-content:space-between;padding:4px 0;font-size:14px;font-weight:700}.grand-total{font-weight:900;font-size:22px;border-top:1px solid #000;padding:10px 0;margin-top:5px}.footer{text-align:center;padding:15px 0;font-size:12px;font-weight:600;border-top:1px dotted #000;margin-top:12px}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}`
...
<div style="padding:${isThermal ? '5px 0' : '12px 0'};font-size:${isThermal ? '11px' : '13px'}"><div><small style="font-weight:700">Invoice:</small> <strong style="font-size:${isThermal ? '12px' : '16px'}">${sale.invoice_no}</strong></div><div><small style="font-weight:700">Date:</small> <strong>${formatDate(sale.created_at)}</strong></div><div><small style="font-weight:700">Customer:</small> <strong>${sale.customer_name}${sale.customer_phone ? ' (' + sale.customer_phone + ')' : ''}</strong></div>${sale.vehicle_no ? `<div><small style="font-weight:700">Vehicle:</small> <strong style="font-size:${isThermal ? '12px' : '16px'};letter-spacing:2px">${sale.vehicle_no}</strong></div>` : ''}</div>
...
${parseFloat(sale.balance_due) > 0 ? `<div style="text-align:right;font-weight:900;font-size:${isThermal ? '14px' : '16px'};margin-top:8px;padding:8px 0;border-top:1px dotted #000">BALANCE DUE: Rs.${parseFloat(sale.balance_due).toLocaleString()}</div>` : ''}
${termsHtml}
