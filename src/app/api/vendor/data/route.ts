...
export async function GET(req: NextRequest) {
  ...
  while (true) {
    const { data } = await admin
      .from('products')
      .select('id, sku, name, description, category, make, model, model_code, year, condition, side, color, oem_code, cost, cost_vat_rate, cost_includes_vat, cost_is_estimate, price, quantity, min_stock_level, parent_product_id, show_price, is_active, vendor_id, created_at, loc_store, loc_floor, loc_sub1, loc_sub2, last_stock_confirmed_at, product_type, tyre_width, tyre_profile, tyre_rim, origin_country, images:product_images(id, url, sort_order)')
      .eq('vendor_id', vendor.id)
  ...
  // WHEEL MART (lk_tax) only — its TabOverview is the sole consumer, so standard
  // vendors (Sakura) pay one settings read here instead of five aggregate queries.
  let dashboard: any = undefined
  const { data: vSettings } = await admin.from('vendor_settings')
    .select('invoice_mode').eq('vendor_id', vendor.id).maybeSingle()
  if (vSettings?.invoice_mode === 'lk_tax') {
