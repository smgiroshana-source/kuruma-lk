...
function printInvoice(sale: any, vendor: any, format: 'a4' | 'thermal', settings?: any) {
  ...
  const a4Style = `@page{size:A4;margin:18mm 15mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#333;font-weight:400;width:100%;max-width:100%;margin:0 auto}.header{text-align:center;padding:15px 0 12px;border-bottom:1px solid #ccc;margin-bottom:12px}.shop-name{font-size:22px;font-weight:700;color:#000}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:0;margin:12px 0;border:1px solid #ddd;border-radius:4px;overflow:hidden}.info-cell{padding:6px 10px;border-bottom:1px solid #eee;font-size:12px}.info-cell:nth-child(odd){background:none}.info-label{color:#888;font-size:10px;text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:1px}.info-value{font-weight:600;color:#000;font-size:13px}table{width:100%;border-collapse:collapse;margin:12px 0;border:1px solid #ddd}th{text-align:left;font-size:11px;font-weight:600;padding:7px 8px;background:none;border:1px solid #ccc;color:#000;text-transform:uppercase;letter-spacing:0.5px}td{padding:8px;font-size:12px;font-weight:400;color:#333;border:1px solid #eee}.text-right{text-align:right}.totals{margin-top:8px}.total-row{display:flex;justify-content:space-between;padding:5px 8px;font-size:13px;font-weight:500;color:#333}.grand-total{font-weight:700;font-size:20px;color:#000;padding:10px 8px;margin-top:4px;border-top:2px solid #333;border-bottom:2px solid #333}.balance-due{font-weight:700;font-size:16px;text-align:right;margin-top:12px;padding:10px;border:2px solid #000;color:#000;border-radius:4px}.footer{text-align:center;padding:20px 0;font-size:11px;color:#999;margin-top:20px;border-top:1px solid #eee}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}`
...
${isThermal ? `<div style="padding:5px 0;font-size:11px"><div><strong>Invoice: </strong><strong style="font-size:12px">${sale.invoice_no}</strong></div><div><strong>Date: </strong><strong>${formatDate(sale.created_at)}</strong></div><div><strong>Customer: </strong><strong>${sale.customer_name}${sale.customer_phone ? ' (' + sale.customer_phone + ')' : ''}</strong></div>${sale.vehicle_no ? `<div><strong>Vehicle: </strong><strong style="font-size:12px;letter-spacing:2px">${sale.vehicle_no}</strong></div>` : ''}</div>` : `<div class="info-grid"><div class="info-cell"><span class="info-label">Invoice No</span><span class="info-value" style="font-size:15px;font-weight:700">${sale.invoice_no}</span></div><div class="info-cell"><span class="info-label">Date</span><span class="info-value">${formatDate(sale.created_at)}</span></div><div class="info-cell"><span class="info-label">Customer</span><span class="info-value">${sale.customer_name}${sale.customer_phone ? ' (' + sale.customer_phone + ')' : ''}</span></div><div class="info-cell"><span class="info-label">Vehicle No</span><span class="info-value" style="font-size:15px;letter-spacing:2px">${sale.vehicle_no || '—'}</span></div></div>`}
...
${paymentLines ? `<div style="margin-top:6px"><div style="font-size:${isThermal ? '10px' : '10px'};font-weight:600;color:${isThermal ? '#000' : '#888'};text-transform:uppercase;letter-spacing:0.5px;margin-bottom:3px">Payments</div>${paymentLines}</div>` : ''}
${sale.notes ? `<div style="margin-top:${isThermal ? '5px' : '8px'};padding:${isThermal ? '4px' : '6px 10px'};font-size:${isThermal ? '10px' : '11px'};font-weight:400;font-style:italic;color:${isThermal ? '#000' : '#666'};background:${isThermal ? 'none' : 'none'};border-left:${isThermal ? 'none' : '2px solid #ccc'};border-radius:3px">Note: ${sale.notes}</div>` : ''}
${(() => {
