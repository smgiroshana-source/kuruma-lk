...
type VendorTab = 'overview' | 'products' | 'add' | 'bulk' | 'pos' | 'sales' | 'credit' | 'stocktake' | 'suppliers' | 'supplier-returns' | 'writeoffs' | 'fleet' | 'cash' | 'reports' | 'settings'
      ...
      <main className={isLkTax ? 'px-6 py-6' : 'max-w-7xl mx-auto px-4 py-6'} style={isLkTax ? {marginLeft:'220px'} : {}}>
                ...
                {filteredProducts.map((p: any) => { const img = (p.images || []).sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0))[0]; const isActive = mobileActiveProduct === p.id; return (
                  <div key={p.id} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                    ...
                    <div className="px-1.5 py-1">
                      ...
                      {p.cost != null && Number(p.cost) > 0 && <span className={'text-[9px] block truncate ' + (isBelowCost(p.price, p.cost) ? 'text-red-600 font-bold' : 'text-slate-400')}>cost Rs.{Number(p.cost).toLocaleString()}{gpPercent(p.price, p.cost) != null ? ' · GP ' + gpPercent(p.price, p.cost) + '%' : ''}</span>}
        ...
        {tab === 'add' && (<div className="max-w-2xl">
          ...
          <form onSubmit={handleAddProduct} className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
            ...
            {parseInt(newProduct.price) > 0 && parseInt(newProduct.cost) > 0 && (isBelowCost(parseInt(newProduct.price), parseInt(newProduct.cost))
              ? <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-[11px] text-red-700 font-bold">⚠️ Selling price (Rs.{parseInt(newProduct.price).toLocaleString()}) is at or below cost (Rs.{parseInt(newProduct.cost).toLocaleString()}) — no profit.</div>
              : <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-[11px] text-green-700">✅ Margin: GP {gpPercent(parseInt(newProduct.price), parseInt(newProduct.cost))}% · profit Rs.{(parseInt(newProduct.price) - parseInt(newProduct.cost)).toLocaleString()}/unit (cost Rs.{parseInt(newProduct.cost).toLocaleString()})</div>)}
