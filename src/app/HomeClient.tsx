...
const SearchInput = memo(function SearchInput({ onSearch, onClear, value }: { onSearch: (q: string) => void; onClear: () => void; value: string }) {
  ...
  // Sync from parent (when parent clears search)
  ...
  function handleChange(val: string) {
    ...
    if (!isMobile) {
      ...
      debounceRef.current = setTimeout(() => onSearch(val), 500)
    }
  }
  ...
  return (
    <div className="relative pb-2.5">
      ...
      <input ref={ref} type="search" enterKeyHint="search" placeholder="Search parts, vehicles, shops..." value={text} onChange={e => handleChange(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { onSearch(text); ref.current?.blur() } }} className="w-full pl-11 pr-10 py-[11px] rounded-[14px] text-[16px] sm:text-sm outline-none bg-[#f7f7f7] text-[#333] transition-all duration-200 border-2 border-transparent focus:bg-white focus:border-[#ff6b35] focus:shadow-[0_0_0_4px_rgba(255,107,53,0.08)]"/>
      {text && <button onClick={() => { setText(''); onClear(); ref.current?.focus() }} className="absolute right-3 top-[9px] w-[26px] h-[26px] bg-[#ddd] rounded-full flex items-center justify-center text-[13px] text-[#666] font-bold active:bg-[#ccc]">✕</button>}
    </div>
  )
})
...
type HomeProps = {
  initialProducts: (Product & { vendor: Vendor; images: any[] })[]
  initialVendors: Vendor[]
  initialSynonyms: string[][]
}

export default function HomePage({ initialProducts, initialVendors, initialSynonyms }: HomeProps) {
  ...
  const [products, setProducts] = useState<(Product & { vendor: Vendor; images: any[] })[]>(initialProducts)
  ...
  const handleSearch = useCallback((q: string) => { setSearch(q); setSearchDisplay(q) }, [])
  const handleSearchClear = useCallback(() => { setSearch(''); setSearchDisplay('') }, [])

  // Background refresh — data comes pre-loaded from server, but refresh in background for freshness
  useEffect(() => { (async () => {
    try {
  ...
  function selectVendor(id: string) { setSelectedVendor(id); setActiveTab('products'); setSelectedCategory('All'); setSearchDisplay(''); setSearch(''); window.scrollTo({top:0,behavior:'smooth'}) }
    ...
    <div className="min-h-screen bg-[#f5f5f5] overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-5 py-4">
        {(activeTab==='products'||isVendorView) && (<div>
          ...
          {isVendorView && <SearchInput onSearch={handleSearch} onClear={handleSearchClear} value={searchDisplay} />}
          ...
          {correctedQuery && allFilteredProducts.length > 0 && (
            <div className="mb-3 px-3 py-2 bg-blue-50 border border-blue-200 rounded-xl text-sm">
              ...
              <button onClick={() => { setSearchDisplay(correctedQuery); setSearch(correctedQuery) }} className="font-bold text-blue-700 underline">{correctedQuery}</button>
            ...
            {wishlistProducts.length===0?(<div className="text-center py-16 px-6"><div className="w-16 h-16 rounded-full bg-[#fef2f2] mx-auto mb-4 flex items-center justify-center text-[24px]">❤️</div><p className="font-bold text-[15px] text-[#333]">Your wishlist is empty</p><p className="text-sm text-[#aaa] mt-1">Tap the heart on any part to save it</p><button onClick={()=>setWishlistOpen(false)} className="mt-5 text-sm font-bold px-6 py-2.5 rounded-xl text-white" style={{background:'linear-gradient(135deg,#ff6b35,#ff8f65)'}}>Browse Parts</button></div>
              ...
              {Object.entries(wishlistByVendor).map(([vid,group])=>(<div key={vid}>
                ...
                <div className="space-y-2">{group.items.map(product=>{const img=getProductImage(product);return(<div key={product.id} className="bg-white rounded-xl border border-[#eee] overflow-hidden flex">
                  <a href={`/product/${(product as any).slug || product.id}`} onClick={()=>setWishlistOpen(false)} className="w-[90px] h-[90px] flex-shrink-0 bg-[#fafafa]">{img?<img src={thumb64(img)} alt={product.name} onError={imgFallback} className="w-[90px] h-[90px] object-cover"/>:<div className="w-[90px] h-[90px] flex items-center justify-center"><span className="text-2xl opacity-10">🔧</span></div>}</a>
                  <div className="flex-1 p-2.5 min-w-0 flex flex-col justify-between">
                    <div>
                      ...
                      <a href={`/product/${(product as any).slug || product.id}`} onClick={()=>setWishlistOpen(false)} className="font-bold text-[12px] text-[#222] leading-tight line-clamp-2 block mt-1">{product.name}</a>
