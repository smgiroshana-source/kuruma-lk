...
export default function VendorDashboard() {
  ...
  // Set primary image (reorder so clicked image becomes sort_order 0)
  async function setPrimaryImage(productId: string, imageId: string, allImages: any[]) {
    ...
    try {
      ...
      const j = await r.json()
      if (j.success) { showToast('Primary image updated'); await fetchData() }
      else showToast('Error: ' + j.error)
    } catch { showToast('Network error') }
  }
