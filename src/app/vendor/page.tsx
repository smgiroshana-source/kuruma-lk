...
<style>@page{size:${isThermal ? '80mm auto' : 'A4'};margin:${isThermal ? '2mm' : '15mm'}}*{margin:0;padding:0;box-sizing:border-box}body{font-family:${isThermal ? "'Courier New',monospace" : "'Segoe UI',Arial,sans-serif"};font-size:${isThermal ? '12px' : '16px'};color:#000;font-weight:${isThermal ? 'normal' : '600'};width:${w}px;max-width:100%;margin:0 auto}.header{text-align:center;padding:${isThermal ? '5px 0' : '20px 0'};border-bottom:${isThermal ? '1px dashed #000' : '2px solid #000'}}.shop-name{font-size:${isThermal ? '16px' : '28px'};font-weight:900;color:#000}table{width:100%;border-collapse:collapse;margin:${isThermal ? '5px 0' : '15px 0'}}th{text-align:left;font-size:${isThermal ? '10px' : '14px'};font-weight:900;color:#000;padding:${isThermal ? '3px 2px' : '10px 5px'};border-bottom:${isThermal ? '1px dashed #000' : '2px solid #000'}}td{padding:${isThermal ? '3px 2px' : '10px 5px'};font-size:${isThermal ? '11px' : '15px'};font-weight:600;color:#000;border-bottom:${isThermal ? '1px solid #ddd' : '1px solid #000'}}.text-right{text-align:right}.totals{${isThermal ? 'border-top:1px dashed #000;' : 'border-top:2px solid #000;'}padding-top:${isThermal ? '5px' : '10px'}}.total-row{display:flex;justify-content:space-between;padding:${isThermal ? '2px 0' : '6px 0'};font-size:${isThermal ? '12px' : '16px'};font-weight:700;color:#000}.grand-total{font-weight:900;font-size:${isThermal ? '16px' : '26px'};color:#000;${isThermal ? 'border-top:1px dashed #000;border-bottom:1px dashed #000;' : 'border-top:2px solid #000;border-bottom:2px solid #000;'}padding:${isThermal ? '5px 0' : '12px 0'};margin-top:5px}.footer{text-align:center;padding:${isThermal ? '8px 0 5px' : '20px 0'};color:#000;font-size:${isThermal ? '10px' : '14px'};font-weight:600;border-top:${isThermal ? '1px dashed #000' : '1px solid #000'};margin-top:${isThermal ? '5px' : '15px'}}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>
<div class="header">${isThermal ? thermalLogoHtml : logoHtml}<div class="shop-name">${shopName}</div>${vendor?.location ? `<div style="font-size:${isThermal ? '10px' : '14px'};color:#000;font-weight:600">${vendor.location}${vendor?.address ? ', ' + vendor.address : ''}</div>` : ''}${vendor?.phone ? `<div style="font-size:${isThermal ? '10px' : '14px'};color:#000;font-weight:600">Tel: ${vendor.phone}${vendor?.whatsapp && vendor.whatsapp !== vendor.phone ? ' | WhatsApp: ' + vendor.whatsapp : ''}</div>` : ''}${taxLine}${emailLine}</div>
<div style="padding:${isThermal ? '5px 0' : '15px 0'};font-size:${isThermal ? '11px' : '15px'}"><div><small style="color:#000;font-weight:700">Invoice:</small> <strong style="font-size:${isThermal ? '12px' : '18px'}">${sale.invoice_no}</strong></div><div><small style="color:#000;font-weight:700">Date:</small> <strong>${formatDate(sale.created_at)}</strong></div><div><small style="color:#000;font-weight:700">Customer:</small> <strong>${sale.customer_name}${sale.customer_phone ? ' (' + sale.customer_phone + ')' : ''}</strong></div>${sale.vehicle_no ? `<div><small style="color:#000;font-weight:700">Vehicle:</small> <strong style="font-size:${isThermal ? '12px' : '18px'};letter-spacing:2px">${sale.vehicle_no}</strong></div>` : ''}</div>
...
${paymentLines ? `<div style="margin-top:8px"><div style="font-size:${isThermal ? '10px' : '13px'};font-weight:900;color:#000;margin-bottom:3px">PAYMENTS</div>${paymentLines}</div>` : ''}
...
${termsHtml}
<div class="footer"><p>${footerText}</p><p style="margin-top:3px;font-size:${isThermal ? '8px' : '11px'};color:#000">Powered by kuruma.lk</p></div></body></html>`
